...
import pamperdata from "../data/Pampers";
...
const Pamperbox = ({ selectedFilter }) => {
  const getFilteredProducts = () => {
    const pampersCol = pamperdata.filter((p) => p.id >= 101 && p.id <= 106);

    if (!selectedFilter) return pampersCol;

    if (selectedFilter.includes("Price")) {
      const numbers = selectedFilter.match(/\d+/g);
      if (numbers && numbers.length === 2) {
        const [min, max] = numbers.map((num) => parseInt(num));
        return pampersCol.filter((p) => p.price >= min && p.price <= max);
      }
      return pampersCol;
    }

    if (selectedFilter.includes("Free Shipping")) {
      return pampersCol.slice(0, 6);
    }

    if (selectedFilter.includes("Discounts")) {
      return pampersCol.filter((p) => p.mrp > p.price);
    }
  ...
  return (
    <div className="flex flex-wrap justify-center gap-20 my-10">
      {filtered.length > 0 ? (
        filtered.map((product) => <Pamper key={product.id} product={product} />)
      ) : (
...
const PamperList = () => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <Pamperbox selectedFilter={selectedFilter} />
