...
import Bottledata from "../data/Bottle";
import FilterMenu from "./FilterMenu";
import '../App.css';

const Bottle = ({ product }) => {
    useEffect(() => {
              document.title = 'Bottles | Earthbuds';
          }, []);
...
const BottleBox = ({ selectedFilter }) => {
  const getFilteredProducts = () => {
     const bottleCol = Bottledata.filter((p) => p.id >= 601 && p.id <= 606);

    if (!selectedFilter) return bottleCol;

    if (selectedFilter.includes("Price")) {
      const numbers = selectedFilter.match(/\d+/g);
      if (numbers && numbers.length === 2) {
        const [min, max] = numbers.map((num) => parseInt(num));
        return bottleCol.filter((p) => p.price >= min && p.price <= max);
      }
      return bottleCol;
    }

    if (selectedFilter.includes("Free Shipping")) {
      return bottleCol.slice(0, 6);
    }

    if (selectedFilter.includes("Discounts")) {
      return bottleCol.filter((p) => p.mrp > p.price);
    }
  ...
  return (
    <div className="flex flex-wrap justify-center gap-20 my-10">
      {filtered.length > 0 ? (
        filtered.map((product) => <Bottle key={product.id} product={product} />)
      ) : (
...
const BottleList = () => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <BottleBox selectedFilter={selectedFilter} />
