...
import Strollerdata from "../data/Stroller";
import FilterMenu from "./FilterMenu";
import '../App.css';

const Stroller = ({ product }) => {
   useEffect(() => {
          document.title = 'Strollers | Earthbuds';
      }, []);
...
const StrollerBox = ({ selectedFilter }) => {
  const getFilteredProducts = () => {
     const strollerCol = Strollerdata.filter((p) => p.id >= 501 && p.id <= 506);

    if (!selectedFilter) return strollerCol;

    if (selectedFilter.includes("Price")) {
      const numbers = selectedFilter.match(/\d+/g);
      if (numbers && numbers.length === 2) {
        const [min, max] = numbers.map((num) => parseInt(num));
        return strollerCol.filter((p) => p.price >= min && p.price <= max);
      }
      return strollerCol;
    }

    if (selectedFilter.includes("Free Shipping")) {
      return strollerCol.slice(0, 6);
    }

    if (selectedFilter.includes("Discounts")) {
      return strollerCol.filter((p) => p.mrp > p.price);
    }
  ...
  return (
    <div className="flex flex-wrap justify-center gap-20 my-10">
      {filtered.length > 0 ? (
        filtered.map((product) => <Stroller key={product.id} product={product} />)
      ) : (
...
const StrollerList = () => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <StrollerBox selectedFilter={selectedFilter} />
